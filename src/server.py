...
from schemes import (GetAdvertisementResponse,
    ...
    UpdateAdvertisementRequest, GetListAdvertisementsResponse)
...
@app.get("/api/v1/advertisement/{ad_id}",
...
@app.patch("/api/v1/advertisement/{ad_id}",
...
@app.delete("/api/v1/advertisement/{ad_id}",
    response_model=DeleteAdvertisementResponse,
    tags=["advertisement"])
async def delete_advertisement(session: SessionDependency, ad_id: int):
    ad = await crud.get_item_by_id(session, Advertisement, ad_id)
